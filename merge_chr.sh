...
awk '{print $1}' human.sam | sort | uniq | wc -l | awk '{print $1*4}'> human_line.txt
...
cd ./human/
./scripts/juicer.sh -D $output_folder/human -y $restriction_file_of_human -s $restriction_enzyme -p $genome_file_of_human -z $reference_file_of_mix -t $thread 2> juicer.log
cd $output_folder
...
cd ./mouse/
./scripts/juicer.sh -D $output_folder/mouse -y $restriction_file_of_mouse -s $restriction_enzyme -p $genome_file_of_mouse -z $reference_file_of_mix -t $thread 2> juicer.log
cd $output_folder
